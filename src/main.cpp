...
#if defined(NDEBUG)
# error "Reactcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x43043007cfbebaafbf41ea1bceacf971512d7fa4a7008649a607ad9a54ef7f81");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Reactcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Reactcoin Signed Message:\n";
...
{
    // Reactcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Reactcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Reactcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Reactcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Reactcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Reactcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x43043007cfbebaafbf41ea1bceacf971512d7fa4a7008649a607ad9a54ef7f81");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "My first react coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04894a1b50b577429b2daf512dae9ee290c990c54fba68493ff82ec3bedd16df9ec56d2494cbaf676696f03ac71e5d36394bb8d71e06a257a1f55dd6f8a8541925") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524608033;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2089186805;
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb36457c59751d83cb6208eadbdaa4d6027e3b14a9cdf5a90623c34155588a707"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Reactcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ReactcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ReactcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ReactcoinMiner(CWallet *pwallet)
{
    printf("ReactcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("reactcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ReactcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ReactcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ReactcoinMiner, pwallet));
}
